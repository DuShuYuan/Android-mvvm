...
object FileUtils {
    ...
    fun listDirsAndFiles(
        ...
        val dirsAndFiles: Array<File?> = arrayOfNulls(dirs.size + files.size)
    ...
    fun getSize(length: Long): String {
        ...
        return when {
            length < KB -> length.toString() + "B"
            length < MB -> df.format(length.toDouble() / KB) + "K"
            length < GB -> df.format(length.toDouble() / MB) + "M"
            else -> df.format(length.toDouble() / GB) + "G"
        }
    }
    /**
     * 获取文件后缀,不包括“.”
